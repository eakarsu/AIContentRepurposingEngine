...
/**
 * Gap/CFS Feature Page: No content approval workflow.
 * Slug: no-content-approval-workflow
 * Backend: /api/gap-no-content-approval-workflow
 */
export default function NoContentApprovalWorkflowPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-content-approval-workflow', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No content approval workflow.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No content approval workflow.</p>

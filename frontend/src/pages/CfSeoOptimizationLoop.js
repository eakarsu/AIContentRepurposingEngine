...
/**
 * Gap/CFS Feature Page: SEO optimization loop
 * Slug: seo-optimization-loop
 * Backend: /api/cf-seo-optimization-loop
 */
export default function SeoOptimizationLoopPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-seo-optimization-loop', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>SEO optimization loop</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Generate variants with different keyword densities/H1 structures; predict organic ranking.</p>

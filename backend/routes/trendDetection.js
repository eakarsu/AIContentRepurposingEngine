/**
 * Custom Feature: trend-detection
 * Real-time trend detection & content suggestions
 *
 * POST /api/ai/trend-detection
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_TREND_DETECTION_KEY
...
const FEATURE_KEY = process.env.FEATURE_TREND_DETECTION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Real-time trend detection & content suggestions.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIContentRepurposingEngine - trend-detection',
    },
    body: JSON.stringify({
...
router.post('/trend-detection', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_TREND_DETECTION_KEY');
    const ai = await callLLM({ feature: 'trend-detection', goal: 'Real-time trend detection & content suggestions', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['trend-detection', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[trend-detection] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'trend-detection',
      endpoint: '/api/ai/trend-detection',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/trend-detection/health', (req, res) => {
  res.json({
    feature: 'trend-detection',
    endpoint: '/api/ai/trend-detection',
